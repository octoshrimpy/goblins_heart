...
var blueberry_harvestable = false
var this_plant = {
...
#	new_item.init_item('blue_berry', 5, 5, 5, 0, 0, 0, 0, 0, true, ITEM_TYPE.CONSUMABLE)
	var blue_berry = Thoth.make_item('blue_berry', 5, 5, 5, 0, 0, 0, 0, 0, true, ITEM_TYPE.CONSUMABLE, harvestable_texture)
	return blue_berry
	
func try_harvest():
	if not this_plant.interactable == true: return
	if not this_plant.has_berries == true: return
	if not blueberry_harvestable == true: return
	if Input.is_action_just_pressed("ui_accept") and this_plant.has_berries == true and this_plant.player_proximity == true and blueberry_harvestable == true:
		harvest_blue_berry()
